...
class ConfigCommandLineRunner implements CommandLineRunner  {
  ...
  public void run(String... args) throws Exception {
	...
	private Iterable<User> createDefaultUsersPlusOrders() {
	    ...
	    // orders
	    Order order1 = new Order("ghijk1234", LocalDate.now().plusWeeks(2), user1); // Agrega el usuario
	    user1.addOrder(order1);

	    // user2
	    User user2 = new User("Kristin", "Solberg");
	    Order order2_1 = new Order("abcde1234", LocalDate.now().plusWeeks(3), user2); // Agrega el usuario
	    Order order2_2 = new Order("qabfde1230", LocalDate.now().plusWeeks(3), user2); // Agrega el usuario
